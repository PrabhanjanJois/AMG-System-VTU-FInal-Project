...
class StudentCourse(models.Model):
    ...
    def get_cie(self):
        ...
        for mk in marks_list:
            m.append(mk.marks1)
        cie = math.ceil(sum(m[:3]) / 3)
        cie += math.ceil(sum(m[3:5]) / 2)
        return cie
...
class Marks(models.Model):
    ...
    def total_marks(self):
        if self.name == 'Event 1':
            return 10
        if self.name == 'Event 2':
            return 10
        if self.name == 'Internal test 1' or 'Internal test 2' or 'Internal test 3':
            return 30
        if self.name == 'Event 1' or 'Event 2':
            return 10
